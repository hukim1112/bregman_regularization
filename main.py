...
def main():
    ...
    params['feature_columns'] = datasets.get_feature_columns(
    ...
    train_filepaths, class_names_to_ids = datasets.load_data(
        FLAGS.train_datadir)
    eval_filepaths, class_names_to_ids = datasets.load_data(FLAGS.eval_datadir)
    datasets.input_fn(train_filepaths, class_names_to_ids,
    ...
    train_spec = tf.estimator.TrainSpec(input_fn=lambda: datasets.input_fn(
        train_filepaths, class_names_to_ids, params['batch_size']), max_steps=FLAGS.max_steps)
    eval_spec = tf.estimator.EvalSpec(input_fn=lambda: datasets.input_fn(
        eval_filepaths, class_names_to_ids, params['batch_size'], step=FLAGS.eval_step))
